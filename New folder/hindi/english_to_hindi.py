...
class EnglishToHindiGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("🇮🇳 English → Hindi Translator")  # Changed title to English to Hindi
        self.root.geometry("1200x750")  # Adjusted to match earlier design
        self.root.configure(bg="#1a1a2e")

        # Local model path (existing English → Hindi model)
        self.model_path = r"C:\Users\intel\models\opus-mt-en-hi"  # Updated model path
        self.model = None
        self.tokenizer = None
    ...
    def build_gui(self):
        # Header
        self.header = tk.Label(self.root, text="🇮🇳 English → Hindi Translator", font=("Helvetica", 24, "bold"), bg="#162447", fg="#e43f5a", pady=15)
        self.header.pack(fill=tk.X)

        # Input Frame
        self.input_frame = tk.Frame(self.root, bg="#1f4068")
        self.input_frame.pack(fill=tk.X, padx=30, pady=(20, 10))
        self.input_label = tk.Label(self.input_frame, text="Enter English Text or Paste Content:", bg="#1f4068", fg="#e0e0e0", font=("Helvetica", 12, "bold"))
        self.input_label.pack(anchor="w", padx=10, pady=(5, 0))
        self.input_text = tk.Text(self.input_frame, height=10, font=("Helvetica", 12), wrap=tk.WORD, bg="#e0e0e0", fg="#1f4068", relief="flat", padx=10, pady=10)
        ...
        self.translate_btn = tk.Button(self.button_frame, text="Translate", bg="#e43f5a", fg="white", font=("Helvetica", 12, "bold"), relief="flat", padx=25, pady=10, command=self.translate_async, state=tk.DISABLED)
        self.translate_btn.pack(side="left", padx=10)

        self.clear_btn = tk.Button(self.button_frame, text="Clear Input", bg="#0f3460", fg="white", font=("Helvetica", 12, "bold"), relief="flat", padx=25, pady=10, command=self.clear_input)
        self.clear_btn.pack(side="left", padx=10)

        self.load_file_btn = tk.Button(self.button_frame, text="Load Word File", bg="#0f3460", fg="white", font=("Helvetica", 12, "bold"), relief="flat", padx=25, pady=10, command=self.load_docx)
        self.load_file_btn.pack(side="left", padx=10)

        self.save_btn = tk.Button(self.button_frame, text="Save Translation", bg="#0f3460", fg="white", font=("Helvetica", 12, "bold"), relief="flat", padx=25, pady=10, command=self.save_translation)
        self.save_btn.pack(side="right", padx=10)

        # Output Frame
        self.output_frame = tk.Frame(self.root, bg="#1f4068")
        self.output_frame.pack(fill=tk.BOTH, expand=True, padx=30, pady=(0, 20))
        self.output_label = tk.Label(self.output_frame, text="Hindi Translation:", bg="#1f4068", fg="#e0e0e0", font=("Helvetica", 12, "bold"))
        self.output_label.pack(anchor="w", padx=10, pady=(5, 0))
        self.output_text = tk.Text(self.output_frame, height=12, font=("Helvetica", 12), wrap=tk.WORD, bg="#e0e0e0", fg="#1f4068", relief="flat", padx=10, pady=10)
        self.output_text.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        self.output_text.insert("1.0", "Translation will appear here...")
        self.output_text.config(state="disabled")

        # Status
        self.status_label = tk.Label(self.root, text="Loading translation model...", bg="#1a1a2e", fg="#00adb5", font=("Helvetica", 10))
    ...
    def load_model_async(self):
        def load_model():
            try:
                ...
                # fallback: try downloading if missing
...
if __name__ == "__main__":
    root = tk.Tk()
    app = EnglishToHindiGUI(root)  # Updated class name
    root.mainloop()
