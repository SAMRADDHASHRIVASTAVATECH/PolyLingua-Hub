...
class JapaneseTranslatorGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("🌸 English → Japanese Translator")
        self.root.geometry("950x650")
        self.root.configure(bg="#1a1a2e")

        # Model configuration - Using FuguMT model (better for English-Japanese)
        self.model_name = "staka/fugumt-en-ja"
        self.model_path = r"C:\Users\intel\models\fugumt-en-ja"
        self.model = None
        self.tokenizer = None
    ...
    def build_gui(self):
        # Header
        self.header = tk.Label(self.root, text="🌸 English → Japanese Translator (FuguMT)",
        ...
        self.input_label = tk.Label(self.input_frame, text="Enter English Text or Paste Content:",
        ...
        self.output_label = tk.Label(self.output_frame, text="Japanese Translation:",
                                     bg="#1f4068", fg="#e0e0e0", font=("Helvetica", 12, "bold"))
        ...
        # Status
        self.status_label = tk.Label(self.root, text="Loading FuguMT translation model...", bg="#1a1a2e",
                                     fg="#00adb5", font=("Helvetica", 10))
        self.status_label.pack(side="bottom", fill=tk.X, pady=5)

    # Load Word file and preserve paragraphs
    def load_docx(self):
    ...
    def load_model_async(self):
        def load_model():
            try:
                self.update_status("Loading FuguMT translation model...")
                ...
                if os.path.exists(os.path.join(self.model_path, "config.json")):
                    try:
                        ...
                        self.update_status(f"Local model error, downloading fresh... Error: {str(e)}")
                
                # Download model from Hugging Face
                self.update_status("Downloading FuguMT model from Hugging Face...")
                self.tokenizer = MarianTokenizer.from_pretrained(self.model_name, cache_dir=self.model_path)
                self.model = MarianMTModel.from_pretrained(self.model_name, cache_dir=self.model_path)
                self.model.to(self.device)
                self.root.after(0, self.model_loaded)
    ...
    def model_loaded(self):
        self.update_status("FuguMT model loaded successfully! Ready to translate.")
        self.translate_btn.config(state="normal")

    def model_error(self, error):
        self.update_status(f"Model load error: {error}")
        messagebox.showerror("Model Error", f"Failed to load FuguMT model:\n{error}")

    # Translate while preserving paragraph structure
    def translate_async(self):
        ...
        def translate():
            try:
                ...
                self.update_status("Translating with FuguMT...")
                ...
                for para in paragraphs:
                    if para.strip():
                        ...
                        inputs = self.tokenizer(para, return_tensors="pt", padding=True, truncation=True, max_length=512)
    ...
    def translation_error(self, error):
        ...
        messagebox.showerror("Translation Error", f"Error during translation:\n{error}")
...
if __name__ == "__main__":
    root = tk.Tk()
    app = JapaneseTranslatorGUI(root)
    root.mainloop()