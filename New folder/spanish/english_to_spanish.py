...
MODEL_DIR = r"C:\Users\intel\models\opus-mt-en-es"
...
def load_model_async(root_window, status_label, translate_button):
    def _load():
        ...
        try:
            ...
            model.to(device_str)
            translator = pipeline("translation_en_to_es", model=model, tokenizer=tokenizer, device=device_index)
...
# ---------------- GUI Build ----------------
root = tk.Tk()
root.title("English → Spanish Translator")
root.geometry("950x650")
root.configure(bg="#1a1a2e")

# Header
header = tk.Label(root, text="🇺🇸 English → Spanish Translator",
...
tk.Label(output_frame, text="Spanish Translation:",
         bg="#1f4068", fg="#e0e0e0", font=("Helvetica", 12, "bold")).pack(anchor="w", padx=8, pady=(6, 0))
